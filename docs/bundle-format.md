...
results, Python wheel publishing, application contract publication, the anonymous pip
index URL, and optional global Git config writes. Python details are also written to
...
When a local interpreter exactly matches a configured Python target, it also records a
temporary venv/pip install against the Gitea index; a missing suitable interpreter is a
documented skip rather than an error.
