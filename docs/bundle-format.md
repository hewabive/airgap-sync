...
`newCommits` when Git can count commits between the previous and fetched ref tips. If
an SSH mirror fails in the parallel batch pass and is retried in an interactive
terminal, its final action also contains `attempts` entries for the batch and
interactive results. Only the final action determines membership in the report's
`errors` array.
...
bundle.
Python settings include the shared application coverage/runtime defaults; omitted
target fields mean inheritance, while present fields are target-local overrides.
An optional target-level `paused: true` records that normal downloads do not refresh
that target. When at least one target is paused, the collector merges newly resolved
bundle documents and indexes with their previously active versions. This keeps the
paused target's existing npm, Git, Python application, and CPython objects live for
pruning without contacting its upstream source.
The merge is intentionally conservative because older bundle documents do not encode
complete per-root ownership; objects no longer needed by a different target may remain
until a full download runs with no pauses.
...
`packages/`, retired `python-packages/` files, and `git-mirrors/`. Dry runs write
`prune-dry-run-report.json`. Pruning is
limited to the transfer bundle and does not remove anything from Verdaccio or Gitea.

Workspace `airgap-sync.json` may also contain operator convenience fields such as the
typed `npmRegistry` target and `giteaUrl`. A Gitea npm target records its owner and
visibility but no duplicate service URL; its endpoint is derived from `giteaUrl`.
Legacy `targetRegistry` strings migrate to `npmRegistry.type: "verdaccio"`. These
fields are used by the interactive menu and are not required for the portable bundle
itself.
...
URL, optional global Git config writes, and the target indexes/object counts omitted
because of publication pauses. Python details are also written to
