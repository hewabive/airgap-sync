...
`workspace-snapshot.json`, when generated by workspace-mode `airgap-sync collect`,
records the configured Git/npm targets that produced the bundle. Git target paths are
stored relative to the workspace, for example `repos/github.com/acme/app`, so later
verification can find the intended project repositories even when the removable media
mount path changes.
...
`apply-report.json`, when generated by `airgap-sync apply`, summarizes the closed-network
...
package manager, command, temporary copy path, exit code, and truncated stdout/stderr.
