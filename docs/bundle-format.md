...
records the configured Git/npm/PyPI targets and Python environment settings that
...
results, Python wheel publishing, the anonymous pip index URL, and optional global Git
config writes. Python details are also written to `python-publish-report.json` (or its
dry-run counterpart). Dry runs write `apply-dry-run-report.json`.
