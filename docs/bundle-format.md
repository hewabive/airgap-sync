...
online orchestration result: optional repository refresh for lower-level directory
collection, npm fetch, Git source metadata, and Git mirror fetch. It also records
fixed-point iterations, including how many Git sources were scanned and how many new
npm/Git requirements were added by mirrored package manifests. The top-level `timings`
object breaks collection time down by repository refresh, root manifest scan, lockfile
scan, npm fetch iterations, Git mirror fetch, Git manifest scan, bundle document
writes, and report writes.
