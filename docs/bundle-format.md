...
The Python portion below documents the current pre-1.0 on-disk format. Plans, locks,
consumer templates, and managed runtime artifacts are transitional implementation
records, not requirements for a machine installing from Gitea PyPI. The intended
...
For schema-v2 `python-app` bundles, this manifest is also a compatibility view over
...
- per-platform `pylock.toml` resolver evidence and a transitional hash-complete
  `requirements.lock`;
...
applications and platform branches without being copied. Current bundles can also
contain optional CPython, `uv`, and license archives. Runtime/tool transfer is being
separated from Python repository coverage and must not change which package closure is
published. The summary reports application/artifact counts and total unique bytes.
...
materialized transitional document paths and digests.
...
results, Python wheel publishing, transitional evidence publication, the Python index
...
When a local interpreter exactly matches a configured Python target, the current
implementation also records a transitional lock-driven venv/pip install against the
Gitea index; a missing suitable interpreter is a documented skip rather than an error.
The intended acceptance checks use ordinary dependency-resolving `pip` and `uv`
installs for every supported compatibility cell.
